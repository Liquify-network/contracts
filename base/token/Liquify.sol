...
contract Liquify is IERC20 {

  string public constant symbol = "LIQUIFY";
  string public constant name = "Liquify token";
  ...
  constructor() {
    minter = msg.sender;
    _mint(msg.sender, 1000000000000000000000000); // 1M initial distribution
  }

  // No checks as its meant to be once off to set minting rights to Minter
  function setMinter(address _minter) external {
    require(msg.sender == minter, "LIQUIFY: Not minter");
    minter = _minter;
  }

  function approve(address _spender, uint _value) external override returns (bool) {
    require(_spender != address(0), "LIQUIFY: Approve to the zero address");
  ...
  function _mint(address _to, uint _amount) internal returns (bool) {
    require(_to != address(0), "LIQUIFY: Mint to the zero address");
  ...
  function _transfer(address _from, address _to, uint _value) internal returns (bool) {
    require(_to != address(0), "LIQUIFY: Transfer to the zero address");

    uint fromBalance = balanceOf[_from];
    require(fromBalance >= _value, "LIQUIFY: Transfer amount exceeds balance");
  unchecked {
  ...
  function transferFrom(address _from, address _to, uint _value) external override returns (bool) {
    ...
    if (spenderAllowance != type(uint).max) {
      require(spenderAllowance >= _value, "LIQUIFY: Insufficient allowance");
    unchecked {
