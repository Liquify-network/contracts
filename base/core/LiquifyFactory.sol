...
import "./LiquifyPair.sol";

contract LiquifyFactory is IFactory {
  ...
  function setPauser(address _pauser) external {
    require(msg.sender == pauser, "LiquifyFactory: Not pauser");
    pendingPauser = _pauser;
  }

  function acceptPauser() external {
    require(msg.sender == pendingPauser, "LiquifyFactory: Not pending pauser");
  ...
  function setSwapFee(address pair, uint value) external {
    require(msg.sender == pauser, "LiquifyFactory: Not pauser");
    LiquifyPair(pair).setSwapFee(value);
  }

  function pairCodeHash() external pure override returns (bytes32) {
    return keccak256(type(LiquifyPair).creationCode);
  }
  ...
  external override returns (address pair) {
    require(tokenA != tokenB, 'LiquifyFactory: IDENTICAL_ADDRESSES');
    (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'LiquifyFactory: ZERO_ADDRESS');
    require(getPair[token0][token1][stable] == address(0), 'LiquifyFactory: PAIR_EXISTS');
    // notice salt includes stable as well, 3 parameters
    bytes32 salt = keccak256(abi.encodePacked(token0, token1, stable));
    (_temp0, _temp1, _temp) = (token0, token1, stable);
    pair = address(new LiquifyPair{salt : salt}());
