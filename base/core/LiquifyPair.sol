...
contract LiquifyPair is IERC20, IPair, Reentrancy {
  ...
  function mint(address to) external lock override returns (uint liquidity) {
    ...
    require(liquidity > 0, 'LiquifyPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock override returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'LiquifyPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
    require(!IFactory(factory).isPaused(), "LiquifyPair: PAUSE");
    require(amount0Out > 0 || amount1Out > 0, 'LiquifyPair: INSUFFICIENT_OUTPUT_AMOUNT');
    (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'LiquifyPair: INSUFFICIENT_LIQUIDITY');
    uint _balance0;
    uint _balance1;
    {// scope for _token{0,1}, avoids stack too deep errors
      (address _token0, address _token1) = (token0, token1);
      require(to != _token0 && to != _token1, 'LiquifyPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'LiquifyPair: INSUFFICIENT_INPUT_AMOUNT');
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), 'LiquifyPair: K');
    }
  ...
  function approve(address spender, uint amount) external override returns (bool) {
    require(spender != address(0), "LiquifyPair: Approve to the zero address");
  ...
  function permit(
    ...
    require(deadline >= block.timestamp, 'LiquifyPair: EXPIRED');
    ...
    require(recoveredAddress != address(0) && recoveredAddress == owner, 'LiquifyPair: INVALID_SIGNATURE');
  ...
  function transferFrom(address src, address dst, uint amount) external override returns (bool) {
    ...
    if (spender != src && spenderAllowance != type(uint).max) {
      require(spenderAllowance >= amount, "LiquifyPair: Insufficient allowance");
    unchecked {
  ...
  function _transferTokens(address src, address dst, uint amount) internal {
    require(dst != address(0), "LiquifyPair: Transfer to the zero address");
    ...
    require(srcBalance >= amount, "LiquifyPair: Transfer amount exceeds balance");
  unchecked {
