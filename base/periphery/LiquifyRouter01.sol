...
contract LiquifyRouter01 {
  ...
  modifier ensure(uint deadline) {
    require(deadline >= block.timestamp, 'LiquifyRouter: EXPIRED');
    _;
  }
  ...
  receive() external payable {
    ...
    require(msg.sender == address(wmatic), "LiquifyRouter: NOT_WMATIC");
  }
  ...
  function _sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
    require(tokenA != tokenB, 'LiquifyRouter: IDENTICAL_ADDRESSES');
    (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'LiquifyRouter: ZERO_ADDRESS');
  }
  ...
  function _quoteLiquidity(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
    require(amountA > 0, 'LiquifyRouter: INSUFFICIENT_AMOUNT');
    require(reserveA > 0 && reserveB > 0, 'LiquifyRouter: INSUFFICIENT_LIQUIDITY');
  ...
  function _getAmountsOut(uint amountIn, Route[] memory routes) internal view returns (uint[] memory amounts) {
    require(routes.length >= 1, 'LiquifyRouter: INVALID_PATH');
  ...
  function _addLiquidity(
    ...
    require(amountADesired >= amountAMin, "LiquifyRouter: DESIRED_A_AMOUNT");
    require(amountBDesired >= amountBMin, "LiquifyRouter: DESIRED_B_AMOUNT");
    // create the pair if it doesn't exist yet
    address _pair = IFactory(factory).getPair(tokenA, tokenB, stable);
    if (_pair == address(0)) {
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      uint amountBOptimal = _quoteLiquidity(amountADesired, reserveA, reserveB);
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, 'LiquifyRouter: INSUFFICIENT_B_AMOUNT');
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint amountAOptimal = _quoteLiquidity(amountBDesired, reserveB, reserveA);
        assert(amountAOptimal <= amountADesired);
        require(amountAOptimal >= amountAMin, 'LiquifyRouter: INSUFFICIENT_A_AMOUNT');
        (amountA, amountB) = (amountAOptimal, amountBDesired);
      }
    }
  }
  ...
  function _removeLiquidity(
    ...
    require(amountA >= amountAMin, 'LiquifyRouter: INSUFFICIENT_A_AMOUNT');
    require(amountB >= amountBMin, 'LiquifyRouter: INSUFFICIENT_B_AMOUNT');
  }
  ...
  function swapExactTokensForTokensSimple(
    ...
    require(amounts[amounts.length - 1] >= amountOutMin, 'LiquifyRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  {
    require(routes[0].from == address(wmatic), 'LiquifyRouter: INVALID_PATH');
  ...
  returns (uint[] memory amounts)
  {
    require(routes[routes.length - 1].to == address(wmatic), 'LiquifyRouter: INVALID_PATH');
    amounts = _getAmountsOut(amountIn, routes);
    require(amounts[amounts.length - 1] >= amountOutMin, 'LiquifyRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    IERC20(routes[0].from).safeTransferFrom(
      msg.sender, _pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    require(
      ...
      'LiquifyRouter: INSUFFICIENT_OUTPUT_AMOUNT'
    );
  }
  ...
  ensure(deadline)
  {
    ...
    require(amountOut >= amountOutMin, 'LiquifyRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  function _safeTransferMATIC(address to, uint value) internal {
    ...
    require(success, 'LiquifyRouter: ETH_TRANSFER_FAILED');
  }
}
